import { yandexLang } from './geocode';
import type { Theme } from './theme';

/**
 * The picker's map, as arithmetic.
 *
 * The map is Yandex's **widget in an `<iframe>`** — the embed anyone may use,
 * with no key and no script on this page. That buys a map that always works and
 * costs the one thing an iframe can never give: nothing inside it is readable
 * from out here. A click in the frame is Yandex's click, a pan in the frame is
 * Yandex's pan, and neither is reportable across the origin.
 *
 * So the frame is not asked. **This app owns the viewport** — the frame is told
 * where to look through its URL, its own controls are made inert, and the pin,
 * the panning and the zoom are drawn and computed here. Which means projecting
 * between coordinates and pixels, which is what this file is.
 *
 ...
 * script on this page. Coordinates go in longitude-first, as they do everywhere
 * in Yandex's APIs.
 *
 * The theme is passed because it cannot be inherited: the frame is another
 * origin and this page's tokens do not reach inside it. Without it a dark page
 * opens a dialog with a white rectangle in the middle of it.
 */
export function mapFrameUrl(view: View, language: string, theme: Theme): string {
 ...
 * this app's frame is deliberately larger than the box it shows through — so
 * that corner is off-screen, clipped along with everything else outside the
 * hole. The credit is therefore put back by hand, as a link that goes where the
...
export function pixelsFrom(view: View, place: { lat: number; lng: number }): {
  x: number;
  y: number;
} {
  const world = worldSize(view.zoom);
...
function yOf(lat: number, zoom: number): number {
  ...
  const scaled =
    Math.tan(Math.PI / 4 + phi / 2) * ((1 - ECCENTRICITY * sin) / (1 + ECCENTRICITY * sin)) ** (ECCENTRICITY / 2);
...
function latAt(y: number, zoom: number): number {
  ...
  for (let pass = 0; pass < 6; pass += 1) {
    ...
    phi =
      Math.PI / 2 -
      2 * Math.atan((1 / scaled) * ((1 - ECCENTRICITY * sin) / (1 + ECCENTRICITY * sin)) ** (ECCENTRICITY / 2));
  }
