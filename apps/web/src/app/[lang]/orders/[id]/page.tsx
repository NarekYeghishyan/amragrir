...
import { OrderStatus, ServiceMode } from '@amragrir/shared';
import { api, ApiError } from '@/lib/api';
import { parseLanguage, t } from '@/lib/language';
import { formatAmd, formatCountdown, formatTime } from '@/lib/format';
import { readSession } from '@/lib/session';
import { ORDER_STATUS_LABEL, TRACKER_STEPS, canCancel, stepIndex } from '@/lib/order-status';
...
export default async function OrderPage({ params, searchParams }: Props) {
  ...
  const live = !cancelled && order.status !== OrderStatus.Completed;

  return (
    <div className="tracking">
      {/* Only while something is still moving — a finished order has nothing
          left to poll for. */}
      {live && <OrderRefresh />}
        ...
        <>
          ...
          <div className="timer">
            {order.scheduled || order.secondsLeft === null ? (
              ...
              <>
                <span className="value">{formatCountdown(order.secondsLeft)}</span>
                <span className="sub">
                  {label('readyIn')}
      ...
      <div className="tracking-actions">
        {/* A plain link, so the page reloads from the server — this is also the
            manual refresh for anyone without JavaScript. */}
        {live && (
