...
import { branchScopeFor } from './branch-access';
import { ListOwnerReservationsDto, SetReservationStatusDto } from '../reservations/dto';

export interface OwnerReservationItem extends ReservationDetail {
...
export class OwnerReservationsService {
  ...
  async list(
    user: JwtPayload,
    query: ListOwnerReservationsDto,
  ): Promise<{ items: OwnerReservationItem[]; total: number; page: number }> {
    const where: Prisma.ReservationWhereInput = { branch: branchScopeFor(user) };
  ...
  async setStatus(
    ...
    return this.reservations.settle(reservation, dto.status, outcome);
  }
}
